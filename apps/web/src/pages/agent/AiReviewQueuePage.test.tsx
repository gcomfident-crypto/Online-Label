...
describe('AiReviewQueuePage', () => {
  ...
  it('点击任务级表格行后从底部打开全页面详情层，并展示多题切换和详情模块', async () => {
    ...
    expect(within(traceSidebar).queryByRole('tablist', { name: '追溯视图' })).not.toBeInTheDocument();
    expect(within(traceSidebar).queryByRole('tab', { name: '当前题' })).not.toBeInTheDocument();
