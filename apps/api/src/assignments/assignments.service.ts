...
type AssignmentsPrismaClient = {
  task: {
    ...
    findUnique: (args: {
      where: { id: string };
      select: { id: true; status: true; quota: true; deadline: true };
  ...
  assignment: {
    count: (args: { where: { taskId: string; status?: { not: 'CANCELLED' } } }) => Promise<number>;
    create: (args: {
      data: {
        taskId: string;
        taskItemId: string;
...
export class AssignmentsService {
  ...
  async claim(input: ClaimAssignmentInput): Promise<ClaimAssignmentDto> {
    ...
    return this.prisma.$transaction(async (client) => {
      ...
      const taskItem = await client.taskItem.findFirst({
        where: {
          taskId: task.id,
          status: 'UNASSIGNED',
        },
      ...
      return toClaimAssignmentDto(assignment, claimedCount + 1);
    });
  }
}
...
function toMarketTaskDto(task: MarketTaskRecord, labelerId?: string): MarketTaskDto {
  ...
  const claimedByMe =
    typeof labelerId === 'string' &&
    labelerId.length > 0 &&
    activeAssignments.some((assignment) => assignment.assigneeId === labelerId);
...
function resolveClaimStatus(input: {
  ...
  return input.remainingCount > 0 ? 'available' : 'full';
}
