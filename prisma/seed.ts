  ...
  return [
    {
      ...
      temperature: 0,
      config: toPrismaJson({ structuredOutputMode: 'function_calling' }),
      enabled: true,
  ...
  return [
    {
      ...
      model: 'mock-stable-reviewer',
      lastError: null,
      logs: toPrismaJson([{ level: 'info', message: 'Mock AI 预审通过。' }]),
...
function createDemoReviewRecords(): Prisma.ReviewRecordUncheckedCreateInput[] {
  return [
    ...
    createReviewRecord({
      ...
      modelMetadata: { provider: 'mock', model: 'mock-stable-reviewer', totalTokens: 128 },
    }),
    createReviewRecord({
    ...
    createReviewRecord({
      ...
      modelMetadata: { provider: 'mock', model: 'mock-stable-reviewer', totalTokens: 112 },
    }),
  ];
}
